...
function App() {
  const { isMinimized, isVisible, activeFeature, setMinimized, setVisible, setActiveFeature, toggle } =
    useFloatingStore()
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('devfloat-toggle', handleGlobalToggle)
  }, [toggle])
  ...
  const FeatureComponent = useMemo(() => FEATURE_COMPONENTS[activeFeature], [activeFeature])
  ...
  return (
    <div className={`app ${isMinimized ? 'app--minimized' : ''}`}>
      <FloatingHeader onMinimize={handleMinimize} onClose={handleClose} />
      {!isMinimized && (
        <>
